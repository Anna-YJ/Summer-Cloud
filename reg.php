...
if($row['uid']){
  ...
  exit('错误：用户名已存在,请重新注册。<a href="javascript:history.back(-1);">返回</a>');
...
$stmt = $dbh->prepare ("INSERT INTO user (username,password,email,regdate) VALUES (:username,:password,:email,:regdate)");
...
if($stmt->execute()){
        ...
        echo '抱歉！添加数据失败：',mysqli_error($dbh),'<br />';
